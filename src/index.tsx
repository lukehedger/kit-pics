...
import { Timeline, type TimelineData } from "./pages/timeline";
...
app.get("/timeline/", async (c) => {
  ...
  const kits = results ?? [];
  const timeline = buildTimeline(kits);
...
function buildStats(
  ...
  const likedByTeam: Record<string, Array<{ alt: string; src: string; team: string }>> = {};
  const likedByYear: Record<string, Array<{ alt: string; src: string; team: string }>> = {};
...
function buildTimeline(kits: Kit[]): TimelineData {
  const typeOrder: Record<string, number> = { Home: 0, Away: 1, Third: 2 };
  const rank = (t: string) => (t in typeOrder ? typeOrder[t]! : 99);
  ...
  for (const team of Object.keys(timeline)) {
    for (const year of Object.keys(timeline[team]!)) {
      timeline[team]![year]!.sort((a, b) => rank(a.type) - rank(b.type));
    }
  }
  return timeline;
}
